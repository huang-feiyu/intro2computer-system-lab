...
cmd_handler(cmd_p) {
    ...
    if (!success) {
        ...
        printf("\n\033[34m%s\033[0m \033[1m=>\033[0m \033[31m%d\033[0m\n", args, val);
    }
    return 0;
...
cmd_handler(cmd_x) {
    ...
    for (int i = 0; i < n; i++) {
        printf("%#x:\t0x%02x\t0x%02x\t0x%02x\t0x%02x\n", addr,
               paddr_read(addr, 1), paddr_read(addr + 1, 1),
               paddr_read(addr + 2, 1), paddr_read(addr + 3, 1));
        addr += 4;
    }
    return 0;
}
...
static struct {
    ...
    {"x", "Scan memory for N bytes from the start", cmd_x},
    {"b", "Set breakpoint", cmd_b},
    {"w", "Set watchpoint", cmd_w},
    {"d", "Delete breakpoint(s).", cmd_d},
    {"cl", "Clear terminal", cmd_cl},
